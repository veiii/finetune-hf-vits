...
def validate_and_preprocess_audio(
    wav_path: str, logger: logging.Logger
    ...
    try:
        ...
        if duration > 30.0:  # Too long for Whisper
            logger.warning(
                f"Audio too long ({duration:.2f}s), truncating to 30s: {wav_path}"
            )
...
def transcribe_single_file(
    args: Tuple[str, whisper.Whisper, logging.Logger, bool],
) -> Optional[Dict]:
    """Transcribe a single audio file with enhanced error handling."""
    wav_path, model, logger, use_file_path = args

    try:
        # If using file path directly, skip preprocessing
        if use_file_path:
            ...
            try:
                ...
                logger.error(f"Failed to read audio file info {wav_path}: {e}")
                return None

            logger.debug(
                f"Transcribing {fname} (duration: {duration:.2f}s) using file path"
            )
            start_time = time.time()

            # Use the file path directly
            result = model.transcribe(
                audio=wav_path, language="pl", fp16=False, verbose=False
            )

        else:
            # Use preprocessed audio data (original behavior)
            ...
            logger.debug(
                f"Transcribing {fname} (duration: {duration:.2f}s) using numpy array"
            )
            start_time = time.time()

            # Use the preprocessed audio data directly
            result = model.transcribe(
                audio=audio_data, language="pl", fp16=False, verbose=False
            )
        ...
        if len(text) < 3:  # Very short transcription might be noise
            logger.warning(
                f"Very short transcription ({len(text)} chars) for {fname}: '{text}'"
            )
            return None

        logger.debug(
            f"Transcribed {fname} in {transcribe_time:.2f}s: '{text[:50]}{'...' if len(text) > 50 else ''}'"
        )
...
def process_batch(
    ...
    args_list = [(path, model, logger, use_file_path) for path in wav_paths]

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_path = {
            executor.submit(transcribe_single_file, args): args[0] for args in args_list
        }
            ...
            for future in pbar:
                ...
                try:
                    ...
                    if result is not None:
                        ...
                        pbar.set_postfix(
                            {"Success": len(records), "Failed": len(failed_files)}
                        )
                    else:
    ...
    if failed_files:
        logger.warning(
            f"Failed to process {len(failed_files)} files: {failed_files[:5]}{'...' if len(failed_files) > 5 else ''}"
        )
...
def transcribe_folder(
    ...
    logger.info(
        f"Starting transcription with model='{model_size}', device='{device}', workers={max_workers}"
    )
    logger.info(
        f"Audio input method: {'file path' if use_file_path else 'numpy array (with preprocessing)'}"
    )
    ...
    # Quick validation pass (lighter validation if using file paths)
    logger.info("Validating audio files...")
    valid_paths = []

    if use_file_path:
        # Lighter validation when using file paths
        for wav_path in tqdm(wav_paths, desc="Validating"):
            try:
                info = sf.info(wav_path)
                duration = info.frames / info.samplerate
                if duration > 0.1:  # Basic duration check
                    valid_paths.append(wav_path)
                else:
                    logger.warning(f"Skipping too short audio: {wav_path}")
            except Exception as e:
                logger.warning(f"Skipping invalid audio file {wav_path}: {e}")
    else:
        # Full preprocessing validation
        for wav_path in tqdm(wav_paths, desc="Validating"):
    ...
    logger.info(
        f"Found {len(valid_paths)} valid audio files ({len(wav_paths) - len(valid_paths)} invalid)"
    )
    ...
    for i in range(0, len(valid_paths), batch_size):
        ...
        batch_records = process_batch(
            batch_paths, model, logger, max_workers, use_file_path
        )
        all_records.extend(batch_records)

        logger.info(
            f"Batch {batch_num} completed: {len(batch_records)}/{len(batch_paths)} files successful"
        )
    ...
    output_records = [
        {k: v for k, v in record.items() if k != "transcribe_time"}
        for record in all_records
    ]

    df = pd.DataFrame.from_records(
        output_records, columns=["audio", "text", "duration", "speaker_id"]
    )
    ...
    avg_transcribe_time = sum(r.get("transcribe_time", 0) for r in all_records) / len(
        all_records
    )
    ...
    logger.info(
        f"✓ Total audio duration: {total_audio_duration:.1f}s ({total_audio_duration/60:.1f}m)"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input-dir", required=True, help="Folder containing chunk_*.wav files"
    )
    parser.add_argument(
        "--output-csv", required=True, help="Path to write metadata CSV"
    )
    parser.add_argument(
        "--model-size",
        default="small",
    ...
    parser.add_argument(
        "--use-file-path",
        action="store_true",
        help="Pass file path directly to Whisper instead of preprocessing audio data",
    )
    ...
    transcribe_folder(
        ...
        use_file_path=args.use_file_path,
    )
